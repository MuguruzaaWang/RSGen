...
from ertsumgraph_transformer import build_optim_bert,build_optim_dec
from ertsumgraph_transformer import ERTSumGraph
...
def train(args,device_id):
    ...
    tokenizer = AutoTokenizer.from_pretrained('/data/run01/scv7414/wpc/rhetorical_aspect_embeddings_myexp/scibert-base')
    padding_idx = tokenizer.pad_token_id
    symbols = {'BOS': tokenizer.vocab['[unused0]'], 'EOS': tokenizer.vocab['[unused1]'],
               'PAD': tokenizer.vocab['[PAD]'], 'EOQ': tokenizer.vocab['[unused2]']}
    vocab_size = tokenizer.vocab_size
    model = ERTSumGraph(args, padding_idx, vocab_size, device, checkpoint)
    ...
    #optim = build_optim(args, model, checkpoint)
    logger.info(model)
    trainer = build_trainer(args, device_id, model, symbols, vocab_size, optim, device)
...
def validate(args, device_id, pt, step):
    ...
    trainer = build_trainer(args, device_id, model, symbols, vocab_size, None, device)
...
def test(args, pt, step):
    ...
    model.eval()
    predictor = build_predictor(args, tokenizer, symbols, model, device, logger=logger)
...
if __name__ == '__main__':
    ...
    # VOCAL_FILE = os.path.join(args.data_dir, "vocab")
    # logger.info("[INFO] Create Vocab, vocab path is %s", VOCAL_FILE)
    # wordvocab = WordVocab(VOCAL_FILE, args.vocab_size)

    # padding_idx = wordvocab.word2id('<PAD>')
    
    train_text_file = os.path.join(args.rhe_data_dir, "train.json")
    test_text_file = os.path.join(args.rhe_data_dir, "val.json")
    ...
    device = "cpu" if args.visible_gpus == '-1' else "cuda"
    args.device =  device
    args.tokenizer = AutoTokenizer.from_pretrained('/data/run01/scv7414/wpc/rhetorical_aspect_embeddings_myexp/scibert-base')
    if (args.mode == 'train'):
        train_dataset = ExampleSet(train_text_file, args.tokenizer, args.sent_max_len, args.doc_max_timesteps, args.docset_max_len, args.device)
        ...
        test_dataset = ExampleSet(test_text_file, args.tokenizer, args.sent_max_len, args.doc_max_timesteps, args.docset_max_len, args.device)
