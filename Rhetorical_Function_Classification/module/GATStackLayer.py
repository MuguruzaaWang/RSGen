...
class MultiHeadLayer(nn.Module):
    def __init__(self, in_dim, out_dim, num_heads, attn_drop_out, feat_embed_size, layer, merge='cat'):
        ...
        for i in range(num_heads): #8 heads
            self.heads.append(layer(in_dim, out_dim, feat_embed_size))  # [n_nodes, hidden_size]  ###一共有8个头，每个头都是一个GAT layer
        self.merge = merge
        self.dropout = nn.Dropout(attn_drop_out) #è®¾ç½®dropout

    def forward(self, g, src, dst):
        head_outs = [attn_head(g, self.dropout(src), self.dropout(dst)) for attn_head in self.heads]  # n_head * [n_nodes, hidden_size]
        if self.merge == 'cat':
            # concat on the output feature dimension (dim=1)
