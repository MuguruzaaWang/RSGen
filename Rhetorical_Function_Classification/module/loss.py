...
class LossComputeBase(nn.Module):
    ...
    def __init__(self, decoder, pad_id):
    ...
    def monolithic_compute_loss(self, batch, outputs):
        """
        ...
        """
        shard_state = self._make_shard_state(batch, outputs)
        _, batch_stats = self._compute_loss(batch, outputs[1]['src'], **shard_state)

        return batch_stats

    def sharded_compute_loss(self, batch, outputs,
                              shard_size,
                             normalization):
        ...
        for shard in shards(shard_state, shard_size):
            loss, stats = self._compute_loss(batch, outputs[1]['src'], **shard)
...
class NMTLossCompute(LossComputeBase):
    ...
    def __init__(self, decoder, symbols, vocab_size,
                 label_smoothing=0.0):
        super(NMTLossCompute, self).__init__(decoder, symbols['PAD'])
        self.sparse = not isinstance(decoder.generator[-1], nn.Softmax)
    ...
    def _make_shard_state(self, batch, output):
        return {
            "output": output[0],
            "target": batch['tgt_extend'][1:],
            "copy_attn": output[1]['attn'],
            "copy_or_generate": output[1]['copy_or_generate']
        }

    def _compute_loss(self, batch, src_words, output, target, copy_attn, copy_or_generate):
        ...
        bottled_src = self._bottle(src_words)

        extra_zeros = batch['extra_zeros']
        if extra_zeros is not None:
            batch_extra, extra_len = extra_zeros.size()
            ###需要扩展extra_len
            extra_zeros = extra_zeros.unsqueeze(0).expand(split_size, batch_extra ,extra_len).contiguous()
        ...
        '''
        scores = self.decoder.get_normalized_probs(bottled_src, bottled_extra_zeros, bottled_output, bottled_copyattn, bottled_cog)
