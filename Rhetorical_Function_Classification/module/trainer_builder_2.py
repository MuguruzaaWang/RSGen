...
class Trainer(object):
            ...
            for i, batch in enumerate(train_iter[0]):
                ...
                if self.n_gpu == 0 or (i % self.n_gpu == self.gpu_rank):
                    ...
                    if accum == self.grad_accum_count:
                        ...
                        normalization = (num_tokens, cls_num, None)
                        self._gradient_accumulation(
                            true_batchs, true_cls_batchs, normalization, total_stats,
                            report_stats)
                        ...
                        examples = 0
                        cls_num = 0
                        if (step % self.args.save_checkpoint_steps == 0 and self.gpu_rank == 0 and step > 12000):
                            self._save(step)
    ...
    def _gradient_accumulation(self, true_batchs, true_cls_batchs, normalization, total_stats,
                               report_stats):
        if self.grad_accum_count > 1:
            self.model.zero_grad()

        for batch, batch_cls in zip(true_batchs, true_cls_batchs):

            src = batch['text']
            tgt = batch['tgt_extend']
            labels = batch_cls['label'].to(src.device, non_blocking=True)

            if self.grad_accum_count == 1:
                self.model.zero_grad()
            
            outputs, pred_labels = self.model(batch, batch_cls)
            #kl_weights = min((self.optim[0]._step + 1) * 1.0 / opt.kl_annealing_steps, 1.0)

            num_tokens,cls_num, _ = normalization
            crossloss = torch.nn.CrossEntropyLoss(reduction='sum')
            loss_cls = crossloss(pred_labels, labels)
            cls_stats = Statistics(cls_loss=loss_cls.clone().item() / float(cls_num)) 
            loss_cls.div(float(cls_num)).backward(retain_graph=True)
            total_stats.update(cls_stats)
            report_stats.update(cls_stats)
