...
def build_optim_cls(args, model, checkpoint):
    """ Build optimizer """
    ...
    if checkpoint is not None:
        ...
        optim = Optimizer(
            args.optim, args.cls_lr, args.max_grad_norm,
            beta1=args.beta1, beta2=args.beta2,
            decay_method=args.decay_method,
            warmup_steps=args.warmup_steps, model_size=args.enc_hidden_size)
...
def train(args,device_id):
    ...
    '''
    symbols = {'BOS':2,'EOS':3,'PAD':1,'UNK':0, 'BOD':4, 'EOD':5}
    model = ERTSumGraph(args, args.word_padding_idx, args.vocab_size, device,symbols, checkpoint)
...
if __name__ == '__main__':
    ...
    VOCAL_FILE = os.path.join(args.data_dir, "vocab2")
    ...
    train_text_file = os.path.join(args.data_dir, "train_wcite_rhelabel.json")
    
    val_text_file = os.path.join(args.data_dir, "val_wcite_rhelabel.json")
    
    test_text_file = os.path.join(args.data_dir, "test_wcite_rhelabel.json")
    ...
    '''
    args.fnames = [train_text_file, val_text_file, test_text_file]
    ...
    if (args.mode == 'train'):
        train_dataset = ExampleSet(train_text_file, wordvocab, args.sent_max_len, args.doc_max_timesteps, args.docset_max_len, args.device)
        #train_sampler = torch.utils.data.distributed.DistributedSampler(train_dataset)
        ...
        valid_dataset = ExampleSet(val_text_file, wordvocab, args.sent_max_len, args.doc_max_timesteps, args.docset_max_len, args.device)
        ...
        test_dataset = ExampleSet(test_text_file,wordvocab, args.sent_max_len, args.doc_max_timesteps, args.docset_max_len,  args.device)
