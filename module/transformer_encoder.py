...
class NewTransformerEncoder(nn.Module):
    ...
    def forward(self, src, lengths=None):
        ...
        #src_features = src_features + topic_emb
...
class TgtTransformerEncoder(nn.Module):
    def __init__(self, num_layers, d_model, heads, d_ff,
        ...
        self.dropout1 = nn.Dropout(dropout)
        self.dropout2 = nn.Dropout(dropout)
        self.layer_norm3 = nn.LayerNorm(self.d_per_head, eps=1e-6)
        self.dropout3 = nn.Dropout(dropout)
        self.feed_forward = PositionwiseFeedForward(d_model, d_ff, dropout)
        self.pooling = MultiHeadedPooling(heads, d_model, dropout=dropout, use_final_linear=False)

    def forward(self, pre_src, starts, ends, lengths=None):
        """ See :obj:`EncoderBase.forward()`"""
        padding_idx = self.embeddings.padding_idx
        if self.training:
            pre_src = pre_src.transpose(0,1)
            batch_size, n_tokens = pre_src.size()
            _, sent_len = ends.size()
            #src = src.unsqueeze(1).expand(batch_size, sent_len, n_tokens)
            src = torch.full([batch_size, sent_len, n_tokens], padding_idx, dtype=pre_src.dtype, device=self.device)
            
            for i in range(batch_size):
                for j, (start, end) in enumerate(zip(starts[i].tolist(), ends[i].tolist())):
                    if start == padding_idx and end == padding_idx:
                        continue
                    src[i][j][:end-start] = pre_src[i][start:end]

        else:
            new_tensor = torch.full([pre_src.shape[0],pre_src.shape[1]],0,device=pre_src.device,dtype=torch.long)
            pre_src = torch.where(pre_src>=self.embeddings.num_embeddings,new_tensor,pre_src)
            src = pre_src.unsqueeze(1)
        batch_size, n_blocks, n_tokens = src.size()
        emb = self.embeddings(src)
